...
void Creature::_processCommand() {
  if (Serial.available()) {
    ...
    if (str.equals("stop")) {
      ...
      Serial.println("sc <instrument> <loop> <transpose> <melody_number> <addr>[ <addr>...]");
      Serial.println("\tSends a sound cage packet\n");

      Serial.println();
    } else if (str.equals("sc")) {
      if (Serial.available()) {
        struct SCPayload p;
        p.instrument = (uint16_t) Serial.parseInt();
        p.loop = (uint8_t) Serial.parseInt();
        p.transpose = (uint8_t) Serial.parseInt();
        p.melody_number = (uint8_t) Serial.parseInt();
        uint8_t i;
        for (i = 0; i < SC_PAYLOAD_MAX_ADDRS; i++) {
          Serial.println("Avail? " + Serial.available());
          if (!Serial.available()) {
            break;
          }
          p.addrs[i] = (uint8_t) Serial.parseInt();
          Serial.println("ID: " + p.addrs[i]);
        }
        p.addrs_len = i;
        bool res = tx(PID_SC_HANDLE, _addr, 255, sizeof(p), (uint8_t *)&p);

        if (!res) {
          Serial.print("ERROR ");
        }
        Serial.println("Sending sc command to all creatures");
        Serial.println(p.instrument);
        Serial.println(p.loop);
        Serial.println(p.transpose);
        Serial.println(p.melody_number);
        Serial.println(p.addrs_len);
        Serial.print("Addrs: ");
        for (i = 0; i < p.addrs_len; i++) {
            Serial.print(p.addrs[i]);
            Serial.print(" ");
        }
        Serial.println();
      }
